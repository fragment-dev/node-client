    ...
    documents.forEach((document) => {
        document.definitions.forEach((definition) => {
            ...
            if (hasLines) {
                ...
                warn(`Operation \`${definition.name?.value}\` posts \`${entryType}\` with Ledger Lines, which a typed payload cannot carry. No payload is generated for it — post it with a raw \`AddLedgerEntryInput\`, which \`addLedgerEntries\` accepts alongside typed payloads.`);
                return;
            }
            ...
            if (typeVersion === undefined) {
                warn(`Operation \`${definition.name?.value}\` binds \`typeVersion\` to a variable, so its version is the caller's to choose. A typed payload names one version and posts it, so none is generated for \`${entryType}\` — post it with a raw \`AddLedgerEntryInput\`, which \`addLedgerEntries\` accepts alongside typed payloads.`);
                return;
            }
            const payload = {
                entryType,
 ...
 */
const renderVariableType = (type, scalars) => type.kind === Kind.NON_NULL_TYPE
